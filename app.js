...
function buildMonthOptions() {
  ...
  const allLabel = monthsWithData.length
    ? `Whole Quarter (${monthsWithData.join(", ")})`
    : "Whole Quarter (no data)";
...
function renderWeeklyChart(weekly) {
  ...
  const achPct = weekly.map((w) => achievementOf(w.achieved, w.target));
  const data = {
    labels: weekly.map((w) => w.name),
    datasets: [
      { type: "bar", label: "Target", data: weekly.map((w) => w.target), backgroundColor: "#C7D3DE", borderRadius: 4, order: 3, yAxisID: "y" },
      { type: "bar", label: "Achieved", data: weekly.map((w) => w.achieved), backgroundColor: GOLD, borderRadius: 4, order: 2, yAxisID: "y" },
      { type: "line", label: "Ach %", data: achPct, borderColor: PLUM, backgroundColor: PLUM,
        borderWidth: 2.5, pointRadius: 4, pointHoverRadius: 6, pointBackgroundColor: PLUM, tension: 0.35, fill: false, order: 1, yAxisID: "y1" },
    ],
  };
...
function render() {
  ...
  renderWeeklyChart(isQuarterView ? calcMonthlySplit(scoped) : calcWeekly(scoped));
