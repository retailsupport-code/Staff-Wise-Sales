...
let state = {
  ...
  store: "All Stores",
  storeOptions: [],      // ["All Stores", ...] built by buildStoreOptions(), used by the prev/next arrows
  storeType: "All Types",
  sortKey: "totalSales",
  sortDir: "desc",
  search: "",
...
function buildStoreOptions() {
  ...
  state.storeOptions = ["All Stores", ...stores];
  if (!state.storeOptions.includes(state.store)) state.store = "All Stores";
  document.getElementById("storeNavLabel").textContent = state.store;
}
...
function renderKPIs(kpis, lyKpis, l2l) {
  ...
  const l2lCardHtml = `
    <div class="kpi-card kpi-card-l2l accent">
      ...
      <div class="l2l-stats">
        <div>
          <span class="l2l-stat-label">Achieved</span>
          <span class="l2l-stat-value">\u20b9${fmtINR(l2l.achieved)}</span>
        </div>
        <div>
          <span class="l2l-stat-label">Store Count</span>
...
function renderWeeklyChart(cySeries, lySeries) {
  ...
  const datasets = [
    ...
    {
      type: "line", label: "Contribution (This Year)", data: cySeries.map((w) => w.achieved),
  ...
  if (hasLY) {
    datasets.push({
      type: "line", label: "Contribution (Last Year)", data: lySeries.map((w) => w.achieved),
...
function render() {
  ...
  document.getElementById("weeklyChartNote").textContent =
    isMultiMonthView ? "" : "\u20b9 across selected scope";
...
function wireEvents() {
  ...
  const stepStore = (dir) => {
    const opts = state.storeOptions.length ? state.storeOptions : ["All Stores"];
    const idx = opts.indexOf(state.store);
    const newIdx = ((idx === -1 ? 0 : idx) + dir + opts.length) % opts.length;
    state.store = opts[newIdx];
    document.getElementById("storeNavLabel").textContent = state.store;
    render();
  };
  document.getElementById("storePrev").addEventListener("click", () => stepStore(-1));
  document.getElementById("storeNext").addEventListener("click", () => stepStore(1));
