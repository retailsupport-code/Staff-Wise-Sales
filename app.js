...
function renderKPIs(kpis, lyKpis, l2l) {
  ...
  const normalCardsHtml = cards.map((c) => `
  ...
  const l2lCardHtml = `
    <div class="kpi-card kpi-card-l2l accent">
      <div class="kpi-label">L2L Stores (trading both years)</div>
      <div class="l2l-stats">
        <div>
          <span class="l2l-stat-label">Target</span>
          <span class="l2l-stat-value">\u20b9${fmtINR(l2l.target)}</span>
        </div>
        <div>
        ...
        <div>
          <span class="l2l-stat-label">Ach %</span>
          <span class="l2l-stat-value">${fmtPct(l2l.achievement)}</span>
        </div>
        <div>
          <span class="l2l-stat-label">Stores</span>
          <span class="l2l-stat-value">${l2l.storeCount}</span>
        </div>
        <div>
          <span class="l2l-stat-label">Growth YoY</span>
  ...
  document.getElementById("kpiRow").innerHTML = normalCardsHtml + l2lCardHtml;
}
...
function renderStoreMatrix(recs) {
  const months = activeMonths();
  const map = new Map();
  recs.forEach((r) => {
    const k = normKey(r.store);
  ...
  const rows = Array.from(map.values()).map((m) => {
    ...
    return { ...m, grossTarget, achievement, atv, bs };
